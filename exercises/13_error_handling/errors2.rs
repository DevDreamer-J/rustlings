...
// Since the player typed in the quantity, we get it as a string.
// They might have typed anything, not just numbers!
//
// Right now, this function isn't handling the error case at all.
// What we want to do is: If we call the `total_cost` function on a string that is not a number, that function will return a `ParseIntError`.
// In that case, we want to immediately return that error from our function and not try to multiply and add.
//
