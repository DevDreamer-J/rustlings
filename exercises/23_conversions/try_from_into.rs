// `TryFrom` is a simple and safe type conversion that may fail in a controlled way under some circumstances. Basically, this is the same as `From`.
// The main difference is that this should return a `Result` type instead of the target type itself.
// You can read more about it in the documentation: https://doc.rust-lang.org/std/convert/trait.TryFrom.html
